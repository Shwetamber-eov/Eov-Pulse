...
class LLMManager:
    def __init__(self):
        ...
        for config in MODEL_CONFIGS:
            ...
            model_states.append(ModelState(config=config, llm=llm,))
    ...
    def invoke_document(self, model, pages, system_prompt: str):
        ...
        while remaining_pages:
            ...
            for idx, chunk in enumerate(chunks):
                ...
                try:
                    response, new_model = self.scheduler.invoke_with_model(model, messages)
    ...
    def invoke_auto(self,pages,system_prompt):
        ...
        if total_tokens <= safe_context:
            ...
            response, model = self.scheduler.invoke_with_model(model,messages,)
            return response.content

        # Large document
        return self.invoke_document(model, pages, system_prompt)
...
def invoke_document(model,pages,system_prompt,):
    return _manager.invoke_document(model,pages,system_prompt,)
